...
class YoutubeMatcher {
    ...
    async matchSpotifyTrack(spotifyTrack) {
        try {
            ...
            query = this.buildSearchQuery(spotifyTrack);
    ...
    fuzzyMatch(spotifyTrack , youtubeResults){
        const searchString = `${spotifyTrack.name} ${spotifyTrack.artists[0].name}`.toLowerCase();
